...
- **Per-department calendar-days tracking.** Opt-in via `departments.time_tracking_mode` (`working_hours` default, or `calendar_days`) + `departments.calendar_days_include_weekends` (default true; when false, Saturday/Sunday are excluded entirely and only Mon–Fri count, each as a full 24h day). Driven by the document's own current `department_id` (not the viewer), so a future cross-office move naturally reverts the display to working-hours once the document leaves an opted-in office. The shared `BusinessHours` engine is untouched for everyone else — this is purely a display-layer branch inside `Document::elapsedSeconds()`. The "Daily Working Time" panel on the document details page is a working-hours-only visualization, so it's **hidden** (not recalculated) whenever the document's department is in `calendar_days` mode.
- **Forward to Department Head.** Opt-in per office via `departments.forward_to_head_enabled`. `Document::departmentHead()` resolves the head by role + `department_id` FK (never by name). `forwardToHead()` hands the document to the head (status `forwarded`, `forwarded_to_head_at` timestamp set, mirrors the existing `forward()` possession semantics — the sender keeps the clock until someone receives). Any other active staff in the same department may `claimFromHead()` instead of waiting on the head specifically — first to act becomes the holder; the head still sees the ordinary Receive button. `Document::isAwaitingHeadClaim()` gates both the `claimFromHead` ability and a `view`/`scopeVisibleTo` carve-out so uninvolved staff in that department can find and see the document before they've ever touched it (they aren't otherwise "concerned" until they claim it).
