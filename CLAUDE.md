...
- **Bulk user delete.** Users index has row checkboxes + a bulk "Delete Selected" bar (Alpine, `x-model="selected"`), gated the same as the per-row Delete button by `Setting::enable_user_delete`. `UserController::bulkDestroy()` always silently skips the requester's own account and any Super Admin account (never deletable in bulk, mirroring `SettingController::resetData()`'s existing "keep Super Admins" convention) — the checkbox for those rows isn't even rendered, so nothing you select is ever silently dropped.
- **Rows-per-page.** Users and Document Tracking both support a `?per_page=` query param (12/25/50/100, validated server-side against that allow-list — anything else falls back to the `records_per_page` Setting) via a `<select onchange="this.form.submit()">` in the filter form. Follow this exact pattern (validated allow-list, not raw user input) for any future per-page dropdown.
- **Notification system.** Super-Admin-only page at `/notifications/settings`. `App\Services\MailSettings::apply()` (called from `AppServiceProvider::boot()`) copies Setting-stored SMTP config onto Laravel's runtime `mail.*` config *only when* `mail_enabled` Setting is `'1'` — until then the app stays on `.env`'s `MAIL_MAILER` (normally `log`), so a half-configured mail setup can never accidentally start sending real email. The SMTP password is the one Setting value that's a real secret, so it's stored **encrypted** via `Crypt::encryptString()`/`MailSettings::setPassword()`/`MailSettings::password()` — never store it as plain Setting text like every other config value. `App\Services\NotificationCatalog::types()` is the single source of truth for which notification types exist (stable string keys, never renamed) — the settings GUI, the per-type enable/frequency Setting (`notification_config` JSON), and the "Run now" test button all iterate this list, so adding a new type is: (1) add an entry to `NotificationCatalog::types()`, (2) write its Mailable in `app/Mail/` + the code that actually triggers it (a scheduled `Schedule::command()` in `routes/console.php` for a digest, or fired inline for an immediate event) — no further GUI changes needed. Mailables are **not** `ShouldQueue`d and sent synchronously — there's no confirmed persistent queue worker on the IIS deployment, so a queued mailable would otherwise sit unsent indefinitely (same reasoning as the Backup module running synchronously). Only one type ships wired up today: `deadline_reminder` (`documents:notify-deadlines`, daily digest, reuses `Document::deadlineHighlight()` so it always matches the tracking-table highlight exactly) — **production needs a Windows Task Scheduler entry running `php artisan schedule:run` every minute** for the daily schedule in `routes/console.php` to actually fire (Windows has no cron).
- **Email templates.** Custom full HTML (table-based, inline styles, `Content(view: ...)`) — **never** the default `x-mail::message` Markdown theme, which looks like generic framework boilerplate, not a professional agency-built email. Always pull branding from `Setting` (`primary_color`, `app_name`, `logo_path`, `organization`, `support_contact`) rather than hardcoding colors/names, so the email matches whatever the office has actually branded the app as. Every notification type should have a `/notifications/settings/preview/{type}` entry (`NotificationSettingController::preview()`) that renders the real Mailable against **fabricated, unsaved model instances** (never real user data) so Super Admin can review the design risk-free before enabling anything.
- **User account reset-to-default-password.** `UserController::resetPassword()` (Edit User page, "Reset Password" card) — for accounts created before the "use default password" toggle existed at Add-User time. Sets the same `User::DEFAULT_PASSWORD` + `must_change_password = true` combo the Add-User toggle uses, so the existing forced-password-change flow (`EnsurePasswordChanged` middleware) picks it up identically — don't build a parallel/different force-change mechanism.
- **Sidebar sub-grouping.** The "Administration" section is organized into labeled sub-groups (People / Document Setup / Work Calendar / System) — smaller `text-[10px]` headers nested under the main `text-[11px]` "Administration" label. Each sub-group header's visibility is computed once via `@php` at the top of the nav (e.g. `$showPeopleGroup = auth()->user()->canAny([...])`) as an OR of that group's own item-level gates — **never hide/show a sub-group header independently of the items inside it**, or an item could show under a hidden header (or vice versa, an empty header with nothing under it).
...
- **`php artisan view:cache` succeeding does NOT mean a Blade file is free of PHP parse errors.** It writes compiled PHP to disk but never executes/parses that output — a `ParseError` (e.g. from a `@if`/`@endif` pair placed inside a Blade **component tag's** attribute list, like `<x-card @if(...) x-data="..." @endif>`, which the component-tag compiler cannot handle) will sit silently cached until a real request renders the page and blows up. Before considering any edited Blade file done, compile it and lint the *compiled output*: `app('blade.compiler')->compileString(file_get_contents($path))`, write that string to a temp `.php` file, then `php -l` that file. Never put `@if`/`@endif` (or any directive) inside a component tag's own attribute list — if an attribute is conditional, either always include it (harmless unused Alpine state costs nothing) or move the conditional to wrap the whole tag instead of splitting its attributes.
- **Alpine checkbox-group arrays must be one consistent type — always strings.** A checkbox's `value` attribute is always a string once rendered into HTML, so `x-model="selected"` on a checkbox always pushes/removes a *string* to/from the array. If a "select all" handler seeds that same array from a PHP collection of IDs via `@js($ids)` without casting (e.g. `@js($ids)` where `$ids` came from `->pluck('id')`), it injects *integers* — and `[5].includes("5")` is `false` in JavaScript, so every row's checked-state silently stops reflecting reality (this shipped once: "select all" visibly checked nothing). Always cast to string before `@js()`: `@js($ids->map(fn ($id) => (string) $id))`.
